...
func Login(args map[string]interface{}) (interface{}, *errors.HccErrorStack) {
	id, idOk := args["id"].(string)
	password, passwordOk := args["password"].(string)

	if !idOk || !passwordOk {
		return model.Token{Token: nil, Errors:*errors.NewHccErrorStack(errors.NewHccError(errors.PiccoloGraphQLArgumentError, "need id and password arguments")).ConvertReportForm()}, nil
	}
	...
	if err != nil {
		...
		return model.Token{Token: nil, Errors:*errors.NewHccErrorStack(loginMismatchError).ConvertReportForm()}, nil
	}
	...
	if err != nil {
		return model.Token{Token: nil, Errors:*errors.NewHccErrorStack(errors.NewHccError(errors.PiccoloGraphQLTokenGenerationError, err.Error())).ConvertReportForm()}, nil
	}

	return model.Token{Token: token, Errors: nil}, nil
}
...
func CheckToken(args map[string]interface{}) (interface{}, error) {
	...
	if !tokenOk {
		return model.Token{Token: nil, Errors:*errors.NewHccErrorStack(errors.NewHccError(errors.PiccoloGraphQLArgumentError, "need a token argument")).ConvertReportForm()}, nil
	}
