...
func ListServer(args map[string]interface{}) (interface{}, error) {
	...
	hccErrStack := errconv.GrpcStackToHcc(&resListServer.HccErrorStack).ConvertReportForm()
...
func NumServer() (interface{}, error) {
	...
	hccErrStack := errconv.GrpcStackToHcc(&resGetServerNum.HccErrorStack).ConvertReportForm()
...
func ServerNode(args map[string]interface{}) (interface{}, error) {
	...
	if !uuidOk {
		return model.ServerNode{Errors: errors.ReturnHccError(errors.PiccoloGraphQLArgumentError, "need a uuid argument")}, nil
	}

	resGetServerNode, err := client.RC.GetServerNode(uuid)
	if err != nil {
		return model.ServerNode{Errors: errors.ReturnHccError(errors.PiccoloGrpcRequestError, err.Error())}, nil
	}
...
func ListServerNode(args map[string]interface{}) (interface{}, error) {
	...
	hccErrStack := errconv.GrpcStackToHcc(&resListServerNode.HccErrorStack).ConvertReportForm()
...
func NumServerNode(args map[string]interface{}) (interface{}, error) {
	...
	hccErrStack := errconv.GrpcStackToHcc(&resGetServerNodeNum.HccErrorStack).ConvertReportForm()
